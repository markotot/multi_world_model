...
class Collector:
    ...
    def add_pregenerated_experience_to_dataset(self, dataset) -> None:

        self.episode_ids = [None] * len(dataset)
        for i, episode in enumerate(dataset):
            episode = Episode(
                observations=torch.ByteTensor(episode['obs']).permute(0, 3, 1, 2).contiguous(),  # channel-first
                actions=torch.LongTensor(episode['actions']),
                rewards=torch.FloatTensor(episode['rewards']),
                ends=torch.LongTensor(episode['dones']),
                mask_padding=torch.ones(episode['dones'].shape[0], dtype=torch.bool),
            )
