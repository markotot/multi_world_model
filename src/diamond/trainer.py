...
class Trainer(StateDictMixin):
    def __init__(self, cfg: DictConfig, root_dir: Path) -> None:
        ...
        dataset_path = "./dataset/storage/breakout_test"
        if os.getcwd().endswith("/diamond"):
            dataset_path = "../" + dataset_path
        self.episodes_dataset = load_dataset(dataset_path, should_split_into_episodes=True) # Temp path so that we don't copy data
