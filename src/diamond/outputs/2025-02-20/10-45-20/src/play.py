...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-p", "--pretrained", action="store_true", help="Download pretrained world model and agent.", default=True)
...
def prepare_play_mode(cfg: DictConfig, args: argparse.Namespace) -> Tuple[PlayEnv, Keymap, ActionNames]:
    ...
    if args.pretrained:
        ...
        hf_path_ckpt = download(f"atari_100k/models/{name}.pt")

        path_ckpt = f"checkpoints/pretrained_diamond/atari_100k/models/{name}.pt"
        current_working_dir = os.getcwd()
        if current_working_dir.endswith("diamond") is False:
            added_path = current_working_dir.split("diamond")[1]
            number_of_subfolders = added_path.count("/")
            path_ckpt = "../" * number_of_subfolders + path_ckpt
    ...
    dataset = Dataset(Path(f"dataset/{name}_{n}"))
