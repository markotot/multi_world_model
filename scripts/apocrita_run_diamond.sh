...
set EXPERIMENT_JOB_PARAMS "-hold_jid ${RUN_NAME}-Pull -N ${RUN_NAME}-Experiment -v JOB_TYPE=\"run_diamond_experiment\",RUN_NAME=$RUN_NAME,START_SEED=$START_SEED,END_SEED=$END_SEED,ENV_NAME=$ENV_NAME,GIT_BRANCH=$GIT_BRANCH,WANDB_API_KEY=$WANDB_API_KEY $PROJECT_NAME/scripts/submit_experiment_job.sh"
